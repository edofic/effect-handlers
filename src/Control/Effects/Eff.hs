...
handleRes :: (Functor e) => Handler e r a b -> Res (e ': r) a -> Res r b
handleRes h (Pure a) = h $ Value a
handleRes h (Free u) = case decomp u of
  Left u -> Free $ fmap (handleRes h) u
  Right  u -> h $ Comp $ fmap (handleRes h) u
...
-- in `e`. Sorry.
handle :: (Functor e) => Handler e r a b -> Eff (e ': r) a -> Eff r b
